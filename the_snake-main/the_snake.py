...
class Snake(GameObject):
    ...
    def move(self):
        ...
        self.position = new_position  # обновляем позицию GameObject
        
        # Удаляем хвост, если не выросли (ИСПРАВЛЕНО: self.length)
        if len(self.positions) > self.length:
            self.positions.pop()
        
        # Проверяем столкновение с собой
        if self.length > 3 and self.positions[0] in self.positions[4:]:
            self.reset()
...
# Функция обработки действий пользователя (ВЫНЕСЕНА ИЗ КЛАССА)
...
def main():
    # Инициализация объектов (ИСПРАВЛЕНО: без параметров)
    snake = Snake()
    apple = Apple()
